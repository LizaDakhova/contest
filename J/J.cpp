...
public:
	...
	void replace(const Vector &);
	bool convex() const;
	bool point_on_edge(const Point &) const;
	bool contains_point(const Point &) const;
	bool crosses_segment(const Segment &) const;
	double square() const;
};
...
double Line::distance_point(const Point &p) const {
	return fabs(a * p.get_x() + b * p.get_y() + c) / sqrt(a * a + b * b);
}
...
Polygon::Polygon(const int amount) {
	this->amount = amount;
	points = new Point[amount]; // возможно, стоит задать все точки нулями
}

Polygon::Polygon(const Polygon &that) {
	this->amount = that.amount;
	this->points = new Point[this->amount];
...
void Polygon::add_point(const Point &p, const int place) {
	points[place] = p;
}

void Polygon::replace(const Vector &v) {
...
bool Polygon::convex() const {
	bool result = true;
	Vector first(points[amount - 2], points[amount - 1]);
	Vector second(points[amount - 1], points[0]);
	double mult = first.vector_mult(second);
	first = second;
	second = Vector(points[0], points[1]);
	if (mult * first.vector_mult(second) < 0)
		result = false;
	else if (!mult)
		mult = first.vector_mult(second);
	for (int i = 0; result && i < amount - 2; ++i) {
		first = second;
		second = Vector(points[i + 1], points[i + 2]);
		if (mult * first.vector_mult(second) < 0)
			result = false;
		else if (!mult)
			mult = first.vector_mult(second);
...
bool Polygon::point_on_edge(const Point &p) const {
	bool result = false;
	Segment edge(points[amount - 1], points[0]);
	if (edge.contains_point(p))
		result = true;
	for (int i = 0; i < amount - 1 && !result; ++i) {
		edge = Segment(points[i], points[i + 1]);
		if (edge.contains_point(p))
			result = true;
	}
	return result;
...
bool Polygon::contains_point(const Point &p) const {
	...
	if (!result) {
		int sign = 1;
		double sum = 0;
		Vector first(p, points[amount - 1]);
		Vector second(p, points[0]);
		if (first.vector_mult(second) < 0)
			sign = -1;
		sum += (acos(first * second / mod(first) / mod(second)) * sign);
		for (int i = 0; i < amount - 1; ++i) {
			sign = 1;
			first = second;
			second = Vector(p, points[i + 1]);
			if (first.vector_mult(second) < 0)
				sign = -1;
			sum += (acos(first * second / mod(first) / mod(second)) * sign);
		}
		result = !(fabs(sum - 0) < 1e-8);
	}
	return result;
}
...
double Polygon::square() const{
	double result = 0;
	Vector first(points[amount - 1].get_x(), points[amount - 1].get_y());
	Vector second(points[0].get_x(), points[0].get_y());
	result += first.vector_mult(second) / 2;
	for (int i = 0; i < amount - 1; ++i) {
		first = second;
		second = Vector(points[i + 1].get_x(), points[i + 1].get_y());
		result += first.vector_mult(second) / 2;
	}
	return fabs(result);
}

bool Polygon::crosses_segment(const Segment &s) const {
	bool result = false;
	if (point_on_edge(s.get_first()) && point_on_edge(s.get_last()))
		result = true;
	else if (contains_point(s.get_first()) && !contains_point(s.get_last()))
		result = true;
	else if (!contains_point(s.get_first()) && contains_point(s.get_last()))
		result = true;
	else
...
int main() {
	int amount;
	std::cin >> amount;
	Polygon polygon(amount);
	Point point;
	for (int i = 0; i < amount; ++i) {
		std::cin >> point;
		polygon.add_point(point, i);
	}
