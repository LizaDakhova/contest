...
BigInt operator/(const BigInt &a, const BigInt &b) {
	...
	//}
	if (abs_a_bigger_b(b, a + BigInt("1"))) { //do not forget about it!!!!
		BigInt result("0");
		return result;
	}
	else {
...
BigInt sqrt(const BigInt &a) {
	...
	BigInt one("1");
	BigInt two("2");
	BigInt avg = (down_edge + up_edge) / two;
	while (abs_a_bigger_b(avg * avg, a + one) || abs_a_bigger_b(a, (avg + one) * (avg + one))) {
		if (abs_a_bigger_b(avg * avg, a - one))
			up_edge = (avg + up_edge) / two;
		else
			down_edge = (avg + down_edge) / two;
		avg = (down_edge + up_edge) / two;
	}
